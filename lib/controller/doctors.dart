...
import 'package:care_4u_m3hadtqani/data/datasource/remote/doctorssss.dart';
import 'package:get/get.dart';
import 'package:url_launcher/url_launcher.dart';

abstract class Doctrorscont extends GetxController {
  intlaData();
  getdata();
...
class DoctrorscontImb extends Doctrorscont {
  MyServices myServices = Get.find();

  String? username;
  int? id;
  DoctorData homeData = DoctorData(Get.find());
  ...
  getdata() async {
    ...
    if (StatusRequest.success == statusRequest) {
      if (response["status"] == "success") {
        doctordata.addAll(response['doctordata']);
      } else {
